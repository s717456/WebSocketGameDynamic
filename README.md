...
這是一個使用 Java Jakarta WebSocket 開發的多人即時 2D 寶石收集遊戲。玩家可以透過瀏覽器進入同一個遊戲場景，移動角色並搶奪寶石。後端使用 WebSocket 維持長連線，接收玩家移動指令後更新遊戲狀態，再即時廣播給所有連線玩家。

我在這個專案中主要練習了 WebSocket 生命週期、多人連線管理、狀態同步、碰撞判斷與 JSON 資料廣播。專案目前使用記憶體保存狀態，未來可以擴充 MySQL 排行榜、會員登入、遊戲房間與更完整的前端互動效果。
